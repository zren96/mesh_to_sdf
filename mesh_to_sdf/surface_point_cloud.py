...
class SurfacePointCloud:
	...
	def sample_sdf_near_surface_with_bounds(self, mesh_bounds, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=11, min_size=0):
		query_points = []
		...
		query_points.append(surface_points + np.random.normal(scale=0.00025, size=(surface_sample_count, 3)))	# was 0.0025
		query_points.append(surface_points + np.random.normal(scale=0.000025, size=(surface_sample_count, 3)))	# was 0.00025

		unit_sphere_sample_count = number_of_points - surface_points.shape[0]*2
		unit_sphere_points = sample_uniform_points_in_unit_sphere(unit_sphere_sample_count, mesh_bounds=mesh_bounds)
